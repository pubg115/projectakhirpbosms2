        ...
        switch (idx) {
            case 0: {
                if (searchBy.equals("")) {
                    ...
                    ArrayList<Customer> dafPelanggan = model.loadDataPelanggan(searchBy, "nama");
                    if (dafPelanggan.isEmpty()) {
                        JOptionPane.showMessageDialog(view, "Pelanggan dengan kata kunci '" + searchBy + "' tidak ditemukan!",
                                "Error", JOptionPane.WARNING_MESSAGE);
                    } else {
            ...
            case 1: {
                ...
                ArrayList<Customer> dafPelanggan = model.loadDataPelanggan(searchBy, "id");
                if (dafPelanggan.isEmpty()) {
                    JOptionPane.showMessageDialog(view, "Pelanggan dengan id '" + searchBy + "' tidak ditemukan!",
                            "Error", JOptionPane.WARNING_MESSAGE);
                } else {
                    cariPelangganView = new CariPelangganView();
                    cariPelangganView.setVisible(true);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source.equals(view.getBtnHitung())) {
            ...
            if (view.getLabelIdCust().equals("null")) {
                ...
                idPelanggan = view.getLabelIdCust(); // Jika pelanggan sudah ada, gunakan ID dari label
            }

            Transaksi transaksiBaru = cek(b, idPelanggan); // Gunakan ID pelanggan yang benar

            if (transaksiBaru != null) {
                model.insertTransaksi(transaksiBaru);
                view.setLabelIdCust("null");
                JOptionPane.showMessageDialog(view, "Berhasil menambahkan transaksi baru",
                        "Sukses", JOptionPane.INFORMATION_MESSAGE);
                showDataTransaksi();
            } else {
                JOptionPane.showMessageDialog(view, "Gagal membuat data transaksi.",
